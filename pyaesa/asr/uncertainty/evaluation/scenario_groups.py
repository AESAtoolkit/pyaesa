"""ASR uncertainty scenario scoped public row grouping."""
...
def _scenario_text_array(series: pd.Series) -> np.ndarray:
    ...
    # Blank or NA scenario cells mean scenario invariant rows, such as
    # historical rows that apply to every scenario.
...
def _scenario_target_positions(
    *,
    ...
    # A scenario row owns its scope. Scenario invariant rows fill only
    # identities without a row for that requested scope.
    max_specificity = (
