"""ASR frequency of transgression figure labels."""
...
FT_FRACTION_COLUMN = "__ft_fraction"
CUMULATIVE_FT_FRACTION_COLUMN = "__cumulative_ft_fraction"
_FT_BOX_GRAPH_GAP_PT = 2.0
_FT_BOX_CONNECTOR_GAP_PT = 0.6


def format_ft_percent(value: object) -> str:
    """Return the shared percent label for ASR frequency of transgression."""
...
def format_ft_math_label(value: object) -> str:
    """Return the polar math label for ASR frequency of transgression."""
    text = format_ft_percent(value)
    escaped = text.replace("%", r"\%")
    return rf"$f^{{\mathrm{{T}}}}={escaped}$"


def ft_legend_entry(*, cc_source: str) -> tuple[Any, str]:
    """Return a figure legend entry explaining ASR frequency of transgression."""
...
def ft_box_legend_entry(*, cc_source: str) -> tuple[Any, str]:
    """Return a legend entry explaining frequency box labels above ASR axes."""
    _handle, label = ft_legend_entry(cc_source=cc_source)
...
def render_ft_box(
    ...
    *,
    ...
    """Render one frequency of transgression box above a subplot."""
    render_ft_boxes(axis, entries=[(float(x), value)], y=y)
...
def render_ft_boxes(
    ...
    *,
    ...
    """Render frequency of transgression boxes above a subplot."""
    render_ft_box_groups([(axis, entries)], y=y)


def render_ft_box_groups(
    groups: Iterable[tuple[Any, Iterable[tuple[float, object]]]],
    *,
    y: float = 1.012,
) -> None:
    """Render frequency of transgression boxes above several subplots."""
    ...
    for axis, entries in active_groups:
        box_entries = [
            (float(x), _add_ft_box_text(axis=axis, x=float(x), value=value, y=y))
            for x, value in entries
        ]
        payloads.append((axis, box_entries))
    renderer = _position_ft_box_groups(fig=fig, groups=payloads)
    for axis, box_entries in payloads:
        for x, text in box_entries:
            axis.plot(
                [float(x), float(x)],
                [1.0, _ft_box_connector_top(axis=axis, text=text, renderer=renderer)],
...
def _position_ft_box_groups(*, fig: Any, groups: list[tuple[Any, list[tuple[float, Any]]]]) -> Any:
    fig.canvas.draw()
    ...
    for axis, box_entries in groups:
        ...
        target_bottom = float(axis_bbox.y1) + _points_to_pixels(fig, _FT_BOX_GRAPH_GAP_PT)
...
def _ft_box_connector_top(*, axis: Any, text: Any, renderer: Any) -> float:
    fig = axis.figure
    bbox = text.get_window_extent(renderer=renderer)
    bottom_y = float(bbox.y0) - _points_to_pixels(fig, _FT_BOX_CONNECTOR_GAP_PT)
