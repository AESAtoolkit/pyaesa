...
def acc_cumulative_identity_groups(
    *,
    identity: pd.DataFrame,
    active_sources: tuple[str, ...],
    dynamic_category_uncertainty_active: bool = False,
) -> tuple[pd.DataFrame, tuple[tuple[str, ...], ...]]:
    """Return yearless cumulative ACC identities and backing yearly row ids."""
    if not _identity_is_dynamic_ar6(identity):
        return identity.iloc[0:0].copy(), ()
    excluded_columns = acc_summary_excluded_columns(
        active_sources=active_sources,
        dynamic_category_uncertainty_active=dynamic_category_uncertainty_active,
    )
    excluded_columns.update({"year", ASOCC_TIME_ROUTE_PUBLIC_COLUMN})
    excluded_columns.difference_update({"l1_l2_method", "l1_method", "l2_method"})
