...
formats, exact summaries, run manifests, completed run reuse, and Sobol
...
| `request/core.py` | Family neutral Monte Carlo request normalization. Omitted or empty `mc_parameters` select convergence defaults; fixed run requests activate the `fixed` block and deactivate the `convergence` block. |
...
| `io/tables.py` | Generic uncertainty table IO, compact run matrices, sparse run rows, public run artifact metadata, and interval index paths. |
| `io/downstream_run_outputs.py` | Family neutral downstream public run table writing, exact public summary scans, and Monte Carlo convergence loops for formulas that consume upstream run tables. |
| `evaluation/summary_groups.py` | Public row grouping and compact or sparse value collapse for downstream uncertainty summaries. |
| `io/summary_kernels.py` | Shared exact summary statistic kernels for bounded float64 value blocks. |
| `io/public_summary.py` | Bounded exact summary and ASR frequency scans from compact or sparse public run artifacts. |
...
| `run_state/runs.py` | Completed run discovery, appendable run selection, and Sobol artifact compatibility checks. |
...
Public Monte Carlo run artifacts follow one shared table contract. In
`csv_compact` output, the artifact path is one CSV file. In Parquet output, the
artifact path is a dataset directory containing `part-*.parquet`
fragments. Each run artifact has a required interval index beside it using the
`*.run_intervals.<suffix>` stem. The interval index records the batch, run
range, row range, and Parquet fragment name needed for bounded scans. Manifests
record the artifact kind and interval index path for each run artifact.
...
random stream construction, public run table writing, bounded exact public
...
helpers. Family code owns prerequisite loading, source specific sample
application, final component figure calls, and public phase ordering.
