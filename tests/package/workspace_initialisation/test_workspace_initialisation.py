...
def test_set_workspace_imports_prerequisites(tmp_path: Path, capsys) -> None:
    ...
    exiobase_grouping_dir = data_raw / "mrio" / "exiobase_3" / "grouping"
    carbon_cov_dir = data_raw / "mrio" / "exiobase_3" / "lcia" / "carbon_accounts_covs"
    oecd_grouping_dir = data_raw / "mrio" / "oecd_v2025" / "grouping"
    ...
    assert (exiobase_grouping_dir / "group_reg_template.csv").exists()
    assert (exiobase_grouping_dir / "group_reg_eu27.csv").exists()
    assert (exiobase_grouping_dir / "group_reg_world.csv").exists()
    assert (exiobase_grouping_dir / "ixi" / "group_sec_elec.csv").exists()
    assert (exiobase_grouping_dir / "ixi" / "group_sec_oecd_d.csv").exists()
    assert (oecd_grouping_dir / "group_reg_fr.csv").exists()
    assert (oecd_grouping_dir / "group_reg_world.csv").exists()
    assert (carbon_cov_dir / "reg_cbca_covs.csv").exists()
    assert (carbon_cov_dir / "reg_cbca_covs_group_eu27.csv").exists()
    assert (carbon_cov_dir / "reg_cbca_covs_group_world.csv").exists()
    assert (carbon_cov_dir / "README_grouped_and_aggregate_lcia_covs.txt").exists()
    ...
    assert (
        packaged_prerequisites_root()
        .joinpath("mrio", "oecd_v2025", "grouping", "group_reg_fr.csv")
        .is_file()
    )
    assert (
        packaged_prerequisites_root()
        .joinpath("mrio", "exiobase_3", "grouping", "ixi", "group_sec_elec.csv")
        .is_file()
    )
    assert (
        packaged_prerequisites_root()
        .joinpath("mrio", "exiobase_3", "grouping", "ixi", "group_sec_oecd_d.csv")
        .is_file()
    )
